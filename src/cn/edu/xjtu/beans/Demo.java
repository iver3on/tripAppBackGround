...
public class Demo {
	...
	public static void main(String[] args) throws NSPException {
		try {
			...
			oauth2Client.initKeyStoreStream(
					Demo.class.getResource("/mykeystorebj.jks").openStream(),
					"123456");
			String appId = "10554894";
			String appKey = "56c4c1d0e0fe0fb33aff1f970c3cc4c4";
			...
			System.err.println("access token :"
					...
					+ ",expires time[access token 过期时间]:"
	 ...
	 * @throws NSPException
	 */
	public static void single_send(NSPClient client) throws NSPException {
		...
		String expire_time = dataFormat
				.format(currentTime + 3 * 60 * 60 * 1000);
		...
		// 接口调用
		PushRet resp = client.call("openpush.message.single_send", hashMap,
				PushRet.class);

		// 打印响应
		System.err.println("单发接口消息响应:" + resp.getResultcode() + ",message:"
				+ resp.getMessage());
	}
	 ...
	 * @throws NSPException
	 */
	public static void batch_send(NSPClient client) throws NSPException {
		// 目标用户列表，必选
		...
		client.setTimeout(10000, 15000);
		// 接口调用
		PushRet resp = client.call("openpush.message.batch_send", hashMap,
				PushRet.class);
	 ...
	 * @throws NSPException
	 */
	public static void lbs_send(NSPClient client) throws NSPException {
		// 地理围栏信息，必选
		// Latlng表示经纬度列表，多个经纬度围城一个封闭区域，必须按照顺序填写，否则会影响定位准确度；
		// 纬度在前，经度在后，纬度、经度之间用英文“,”分隔。
		// Type表示封闭区域类型，目前仅支持1，表示多边形
		String location = "{\"location\":{\"type\":\"1\",\"coordinates\":[{\"lat\":\"32.57844\",\"lng\":\"117.502087\"},{\"lat\":\"30.848528\",\"lng\":\"118.458746\"},{\"lat\":\"31.33934\",\"lng\":\"120.316873\"},{\"lat\":\"32.656293\",\"lng\":\"119.580981\"}]}}";
		...
		client.setTimeout(10000, 15000);
		// 接口调用
		String rsp = client.call("openpush.openapi.lbs_send", hashMap,
				String.class);
